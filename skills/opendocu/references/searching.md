...
5. If there are many plausible results, use `--json` and inspect `heading_path`, `url`, `version`, and `snippet`.

## Result Discipline

Prefer results that match the requested version, official source URL, and exact API section heading. Use `opendocu get` for full context before generating code when the snippet omits caveats, stability notes, version history, parameters, or return values.
